...
export default function NewPurchase() {
  ...
  const [showConfirmModal, setShowConfirmModal] = useState(false); // NUEVO
  ...
  const registrarCompra = async () => {
    ...
    try {
      ...
      setShowNotification(true);  // Mostrar éxito
      setProductosEnCompra([]); 
      setProveedor(''); 
      setAlmacen('');
  ...
  return (
    <div className="bg-slate-50/50 min-h-screen font-sans pb-32 text-slate-600">
      ...
      {/* --- NUEVO: MODAL CONFIRMACIÓN DE GUARDADO --- */}
      {showConfirmModal && (
        <div className="fixed inset-0 z-[140] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 animate-zoomIn">
            
            <div className="text-center mb-6">
              <div className="w-16 h-16 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center mx-auto mb-4">
                <IoMdCheckmark className="text-3xl" />
              </div>
              <h3 className="text-xl font-bold text-slate-800 mb-1">Confirmar Registro</h3>
              <p className="text-slate-500 text-sm">¿Estás seguro de registrar esta compra por <br/> <strong className="text-slate-800">S/ {totalAPagar.toFixed(2)}</strong>?</p>
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                {isSaving ? 'Guardando...' : 'Sí, Guardar Compra'}
              </button>
              <button 
                onClick={() => setShowConfirmModal(false)} 
                className="w-full py-3.5 text-slate-500 font-bold hover:bg-slate-50 rounded-xl transition-colors"
              >
                Revisar detalles
              </button>
            </div>
